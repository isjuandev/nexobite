...
description: An inspired interpretation of Supabaze's design language — an open-source database platform built on a clean white-and-near-black system with a single signature orange CTA, a custom humanist sans display tier, and dense product UI mockups composited above the hero. The brand reads as quietly technical: minimal chrome, a near-monochrome palette, and the orange primary acting as the only chromatic event on the page.
...
**Key Characteristics:**
- Single orange primary (`{colors.primary}` `#FF7400`) as the only chromatic event; everything else is monochrome.
...
### Brand & Accent
- **Orange** (`{colors.primary}` — `#FF7400`): The signature CTA color. Filled-button background, brand wordmark accent, dot indicator.
- **Orange Deep** (`{colors.primary-deep}` — `#CC5D00`): Pressed-state lift of the primary.
- **Orange Soft** (`{colors.primary-soft}` — `#FF9A4D`): Lighter orange used in chart accents and product UI.
...
- **On Primary** (`{colors.on-primary}` — `#171717`): Text on the orange primary fill — near-black, not white. The button reads as a "lit" surface with dark type, not a colored chip.
...
**`pill-tag-orange`** — small orange pill used for "new" or featured indicators.
...
### Do
- Reserve `{colors.primary}` orange for filled CTAs and the wordmark accent — it should appear sparingly.
...
- Use near-black `{colors.ink}` on the orange button (not white) — the orange reads as "lit" with dark type, which is the brand's idiosyncratic choice.
...
- Don't use white text on the orange button — the brand specifically uses near-black on orange.
...
5. Keep orange scarce; one filled orange button per viewport.
